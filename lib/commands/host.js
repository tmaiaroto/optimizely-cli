...
var callTaskRunner = function(varPath, program) {
  var runners = ["brocoli", "grunt", "gulp"];
  var runnerFiles = ["brocfile.js", "gruntfile.js", "gulpfile.js"]
  ...
  if(runners.indexOf(runner) === -1){
    ...
    for(var i = 0; i < runnerFiles.length; i++){
      if(fs.existsSync(path.resolve(taskPath, runnerFiles[i])) ){
        runner = runners[i];
        break;
      }
    }
...
module.exports = function(varPath, port, program) {
  ...
  if(program.task){
    callTaskRunner(varPath, program);
  }
  port = port || 8080
  //Start Server
  var app = express();
  ...
  var serverConnected = function(){
      ...
      if(program.live){
        ...
        console.log('Live reload enabled for %s', path.resolve("..",varPath));
      }
